...
class Relation_Classification(nn.Module):
    def __init__(self, relation_dim):
        ...
        self.linear = nn.Linear(2*self.ner_embedding+self.BiLSTM_hidden_size+self.conv_out_dim+self.position_dim, tmp_dim)
        self.linear2 = nn.Linear(tmp_dim, self.relation_dim)

        #self.conv_5 = nn.Conv1d(self.BiLSTM_hidden_size,self.conv_out_dim,5,1,2)
        #self.conv_5r = nn.Conv1d(self.BiLSTM_hidden_size,self.conv_out_dim,5,1,2)
    ...
    def forward(self, src_event,src,middle,dst,dst_event,reverse_flag,middle_flag):
        ...
        src_event = self.pooling(src_event)
        s_m_d = torch.cat((src,middle,dst),0).permute(1,2,0)  # (L1+L2+L3)*N*BiLSTM_hidden_size -> N*BiLSTM_hidden_size*(L1+L2+L3)
        s_m_d_1 = self.pooling(s_m_d)
        dst_event = dst_event.permute(1,2,0)
        dst_event = self.pooling(dst_event)

        if not reverse_flag :
            s_m_d_2 = self.conv_3(s_m_d) ###
            s_m_d_2 = self.pooling(s_m_d_2)
        else :
            s_m_d_2 = self.conv_3r(s_m_d) ###
            s_m_d_2 = self.pooling(s_m_d_2)

        x = torch.cat((src_event,s_m_d_1,s_m_d_2,dst_event,pe),1) # N * 3 BiLSTM_hidden_size * 1
        x = x.view(1,-1) # N(=1) * 3 BiLSTM_hidden_size
        #x = self.layer_norm(x)
        x = self.dropout(x)
        x = F.leaky_relu(self.linear(x)) # N*relation_dim
